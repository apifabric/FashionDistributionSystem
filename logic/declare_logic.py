...
def declare_logic():
    ...
    # Calculates Designer balance based on unpaid orders.
    Rule.sum(derive=Designer.balance, as_sum_of=Order.amount_total, where=lambda row: row.date_shipped is None)

    # Calculates total revenue for a collection of designs based on sales.
    Rule.sum(derive=Collection.total_revenue, as_sum_of=Design.revenue)

    # Counts total number of designs under a brand.
    Rule.count(derive=Brand.number_of_designs, as_count_of=Design)
